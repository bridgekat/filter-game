import FilterGame.Solutions.Basis
...
-/
theorem Filter.as_basis_le_iff (f : Filter α) (b : Basis α) :
    f.as_basis ≤' b ↔ f ≤ b.as_filter := by
  apply Iff.intro
  · intros h
...
-/
theorem Basis.le_and_le_iff_filter_eq (b c : Basis α) :
    b ≤' c ∧ c ≤' b ↔ b.as_filter = c.as_filter := by
  apply Iff.intro
  · intros h
    apply le_antisymm
    · exact Basis.as_filter_le h.1
    · exact Basis.as_filter_le h.2
  · suffices h : ∀ b c, Basis.as_filter b = Basis.as_filter c → b ≤' c by
      intros heq
